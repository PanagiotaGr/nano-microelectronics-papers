#!/usr/bin/env python3
"""
Nano & Microelectronics ArXiv Daily

- Queries arXiv API by category.
- Filters into topic buckets with keyword matching.
- Optional domain gate reduces noise.
- Writes:
  1) digests/YYYY-MM-DD.md
  2) topics/<topic>.md
  3) README.md Today block inside markers
...
def arxiv_query(category: str, max_results: int = 300) -> List[Paper]:
    params = {
...
def haystack(p: Paper, mode: str) -> str:
    if mode == "title":
        return normalize(p.title)
    return normalize(p.title + " " + p.summary)


def passes_gate(p: Paper, mode: str, must_have_any: Optional[List[str]]) -> bool:
    if not must_have_any:
        return True
    h = haystack(p, mode)
    return any(normalize(t) in h for t in must_have_any if normalize(t))


def match_keywords(p: Paper, keywords: List[str], mode: str) -> List[str]:
    h = haystack(p, mode)
    return [kw for kw in keywords if (normalize(kw) and normalize(kw) in h)]
...
def format_paper_md(p: Paper, matched: List[str]) -> str:
    ...
    tags = ", ".join(matched[:10]) if matched else ""
...
def write_daily_digest(today: str, sections: Dict[str, List[str]]) -> str:
    ...
    lines = [f"# Daily Digest — {today}", "", "Auto-generated from arXiv using topic keyword filters.", "> Edit `config.yml` to adjust topics/keywords and limits.", ""]
...
def write_topics(today: str, sections: Dict[str, List[str]]) -> Dict[str, str]:
    ...
    for topic, items in sections.items():
        ...
        lines = [f"# {topic}", "", f"**Last update:** {today}", "", "> Auto-generated. Edit `config.yml` to change keywords/topics.", ""]
        if not items:
            lines.append("_No matches today._")
        else:
            lines.append("## Latest\n")
        ...
        lines += ["", "---", "", "Back to main page: [README](../README.md)"]
        with open(path, "w", encoding="utf-8") as f:
            f.write("\n".join(lines).strip() + "\n")
    return topic_paths
...
def main() -> None:
    ...
    per_topic_cap = int(cfg.get("max_results_per_topic", 15))
    categories = list(cfg.get("categories", []))
    topics: Dict[str, List[str]] = cfg.get("topics", {})
    mode = cfg.get("match_in", "title+abstract")
    must_have_any = cfg.get("must_have_any", None)
    ...
    for cat in categories:
        try:
            papers = arxiv_query(cat, max_results=300)
            for p in papers:
                if is_recent(p, days_back=days_back) and passes_gate(p, mode, must_have_any):
                    recent.append(p)
    ...
    for base_id, p in uniq.items():
        ...
        for topic, kws in topics.items():
            matched = match_keywords(p, kws, mode)
